...
4.  **形式**: `{{c1::キーワード}}` `{{c2::キーワード}}` ... の形式を使います。
...
class LLMHandler:
    ...
    def convert_cards(self, cards_batch):
        ...
        for single_card_line in cards_batch:
            ...
            try:
                ...
                print(error_message)
                results.append(f"エラー: {single_card_line} -> {e}")
    ...
    def _convert_with_deepseek(self, prompt):
        ...
        try:
            response = self.session.post("https://api.deepseek.com/v1/chat/completions", headers=headers, json=payload, timeout=30)
    ...
    def _convert_with_ollama(self, prompt):
        ...
        try:
            response = self.session.post("http://localhost:11434/api/generate", json=payload, timeout=30)
            response.raise_for_status()
            result = response.json()
