...
8.  **最重要出力ルール**: **指定されたタブ区切り形式以外の、いかなるテキスト（解説、挨拶、補足情報など）も絶対に出力してはいけません。**
    ...
    def convert_cards(self, cards_batch):
        ...
        for single_card_line in cards_batch:
            ...
            try:
                ...
                # resultがNoneでないことを確認してyieldする (空文字列は許容)
                if result is not None:
                    yield result
                else: # 予期せぬNoneの場合
                    yield f"エラー: カード「{single_card_line}」の処理中に予期せぬNone応答がありました。"


            except Exception as e:
                error_message = f"カード「{single_card_line}」の処理中にエラー: {e}"
                print(error_message)
                # エラーが発生した場合は空文字を返し、UI側で警告として処理させる
                yield ""
