...
3.  **形式**: `{{c1::キーワード}}` `{{c2::キーワード}}` ... の形式を使います。
4.  **出力**: `新しい穴埋め文（オモテ面）\t元の答え（ウラ面）` のタブ区切り形式で出力します。ウラ面には元の答えをそのまま残してください。
5.  **言語**: 絶対に日本語のまま処理し、英語に翻訳してはいけません。
6.  **その他**: 説明文（「以下が出力です」など）は一切不要です。
