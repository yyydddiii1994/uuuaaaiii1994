...
class LLMHandler:
    def __init__(self, config, cancel_event=None):
        ...
        # configからmax_workersを取得、デフォルトは4
    ...
    def convert_cards(self, cards_batch):
        """
        複数のカードを並列処理します。
        健司戦略（カードごとの個別処理）を維持しつつ、ThreadPoolExecutorで高速化します。
        """
        if self.cancel_event and self.cancel_event.is_set():
            return

        futures = {self.executor.submit(self._process_single_card, card): card for card in cards_batch}
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if result is None: # キャンセルされたタスク
                    continue

                if isinstance(result, dict) and "error" in result:
                    # エラー情報を生成
                    yield f"エラー: {result['card']} -> {result['error']}"
                else:
                    # 成功した結果を生成
                    yield result
                ...
                card = futures[future]
                error_message = f"カード「{card[:30]}...」の処理中に予期せぬエラー: {e}"
                print(error_message)
                yield f"エラー: {card} -> {e}"

    def _process_single_card(self, single_card_line):
        """単一のカードを処理する内部メソッド"""
        if self.cancel_event and self.cancel_event.is_set():
            return None
        ...
        try:
            ...
            print(error_message)
            # エラーは特別な形式で返す
            return {"error": error_message, "card": single_card_line}

    def shutdown(self):
        """ThreadPoolExecutorを安全にシャットダウンする。"""
        print("LLMハンドラのシャットダウンを開始します...")
        self.executor.shutdown(wait=False)
        print("LLMハンドラのシャットダウンが完了しました。")
