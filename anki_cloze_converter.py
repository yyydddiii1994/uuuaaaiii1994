...
class TextRedirector(object):
    ...
    def write(self, str_in):
        self.line_buffer += str_in
        if '\n' in self.line_buffer:
            lines = self.line_buffer.split('\n')
            for line in lines[:-1]:
                if line.strip(): # 空行はタイムスタンプを付けない
                    timestamp = datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")
                    self.widget.config(state=tk.NORMAL)
                    self.widget.insert(tk.END, f"{timestamp} {line}\n")
                    self.widget.see(tk.END)
                    self.widget.config(state=tk.DISABLED)
            self.line_buffer = lines[-1]

    def flush(self):
        # バッファに残っている内容があれば書き出す
        if self.line_buffer:
            timestamp = datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")
            self.widget.config(state=tk.NORMAL)
            self.widget.insert(tk.END, f"{timestamp} {self.line_buffer}\n")
            self.widget.see(tk.END)
            self.widget.config(state=tk.DISABLED)
            self.line_buffer = ""
...
class AnkiClozeConverter(tk.Tk):
    def __init__(self):
        ...
        self.timer_id = None
        self.handler = None # LLMHandlerのインスタンスを保持
        ...
        # --- 並列処理設定の追加 ---
        performance_frame = ttk.LabelFrame(settings_tab, text="パフォーマンス設定", padding=10)
        performance_frame.pack(fill=tk.X, padx=10, pady=10, side=tk.TOP)

        max_workers_label = ttk.Label(performance_frame, text="並列処理数 (スレッド数):")
        max_workers_label.grid(row=0, column=0, sticky="w", padx=5, pady=5)

        # CPUコア数を参考にデフォルト値を設定
        import os
        default_workers = os.cpu_count() or 4
        self.max_workers_spinbox = ttk.Spinbox(performance_frame, from_=1, to=32, increment=1, width=10)
        self.max_workers_spinbox.set(default_workers)
        self.max_workers_spinbox.grid(row=0, column=1, padx=5, pady=5)

        performance_info_text = f"変換処理を同時に実行する数。お使いのPCのCPUコア数に合わせるのがおすすめです (推奨: {default_workers})。\n値を大きくしすぎると、PC全体の動作が不安定になる可能性があります。"
        performance_info_label = ttk.Label(performance_frame, text=performance_info_text, wraplength=500, justify=tk.LEFT)
        performance_info_label.grid(row=1, column=0, columnspan=2, sticky="w", padx=5, pady=5)
        ...
        bottom_frame.columnconfigure(2, weight=1) # Status label
        bottom_frame.columnconfigure(3, weight=1) # Progress bar
        ...
        self.progress_bar.grid(row=0, column=3, sticky="ew", padx=(0, 10))

        self.elapsed_time_label = ttk.Label(bottom_frame, text="経過時間: 00:00")
        self.elapsed_time_label.grid(row=0, column=4, padx=(0, 5))

        self.eta_label = ttk.Label(bottom_frame, text="残り推定: --:--")
        self.eta_label.grid(row=0, column=5, padx=(0, 5))
    ...
    def conversion_thread(self):
        ...
        try:
            ...
            max_workers = int(self.max_workers_spinbox.get())
            ...
            while i < len(lines):
                ...
                if '\t' in line:
                    ...
                    cards.append(f"{lines[i]}\t{lines[i+1]}")
                    i += 2
                else:
                    print(f"警告: ペアにならない最後の行「{line}」は無視されます。")
                    i += 1
            ...
            # LLMハンドラを初期化
            self.handler = LLMHandler(config, self.cancel_event)

            processed_cards = 0
            self.queue.put(("status", f"合計 {total_cards} 枚のカードを、{max_workers} の並列処理で変換開始..."))

            # 全カードを一度にハンドラに渡し、並列処理させる
            for result in self.handler.convert_cards(cards):
                if self.cancel_event.is_set():
                    break

                self.queue.put(("result_part", result))
                processed_cards += 1
    ...
    def start_timer(self):
        ...
        # 初回のUIリセット
        self.elapsed_time_label.config(text="経過時間: 00:00")
        self.eta_label.config(text="残り推定: --:--")
        self.update_timer()
    ...
    def update_timer(self):
        ...
        processed_cards = self.progress_bar['value']
        total_cards = self.progress_bar['maximum']

        eta_str = "--:--"
        if processed_cards > 5 and elapsed_seconds > 5: # 安定した推定のために少し待つ
            time_per_card = elapsed_seconds / processed_cards
            remaining_cards = total_cards - processed_cards
            remaining_seconds = int(time_per_card * remaining_cards)
            eta_str = time.strftime('%M:%S', time.gmtime(remaining_seconds))

        self.eta_label.config(text=f"残り推定: {eta_str}")
    ...
    def stop_timer(self):
        if self.timer_id:
            ...
            self.timer_id = None
        # self.start_time = None # 最後の経過時間を表示するためにコメントアウト
    ...
    def update_ollama_context_settings(self, model_name):
        ...
        if details and 'parameters' in details:
            ...
            # 正規表現を使ってnum_ctxの値をより安定的に抽出
            match = re.search(r"num_ctx\s+(\d+)", params_str)
            if match:
                try:
                    default_ctx = int(match.group(1))
                    min_ctx = 2048
                    max_ctx = max(default_ctx * 2, 8192)
                    print(f"モデル '{model_name}' のデフォルトコンテキスト長: {default_ctx}")
                except (ValueError, IndexError):
                    print(f"モデル '{model_name}' のnum_ctxの解析に失敗しました。デフォルト値を使用します。")
            else:
                print(f"モデル '{model_name}' のパラメータからnum_ctxが見つかりませんでした。デフォルト値を使用します。")
        else:
    ...
    def setup_tooltips(self):
        ...
        ollama_tooltip_text = "Ollamaで利用するローカルモデルを選択します。\n【高速化のヒント】RTX 4070のようなNVIDIA GPUをお持ちの場合、\nOllamaがGPUで動作しているか確認してください。\nタスクマネージャーでGPU使用率が上昇していれば成功です。"
        Tooltip(self.ollama_model_selector, ollama_tooltip_text)
    ...
    def save_config(self):
        config_data = {
            ...
            "max_workers": self.max_workers_spinbox.get()
        }
        try:
    ...
    def load_config(self):
        ...
        try:
            ...
            # 並列処理数を読み込み、見つからなければデフォルト値を設定
            default_workers = os.cpu_count() or 4
            self.max_workers_spinbox.set(config_data.get("max_workers", default_workers))
    ...
    def on_closing(self):
        if self.handler:
            print("アプリケーション終了のため、LLMハンドラをシャットダウンします。")
            self.handler.shutdown()
        self.save_config()
        self.destroy()
