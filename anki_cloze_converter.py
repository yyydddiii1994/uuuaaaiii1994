...
class AnkiClozeConverter(tk.Tk):
    ...
    def process_queue(self):
        try:
            ...
            if message[0] == "status":
                ...
                result, original_card = message[1], message[2]
                self.output_text.config(state=tk.NORMAL)

                if result.strip():
                    self.output_text.insert(tk.END, result + "\n")
                else:
                    # 結果が空文字列の場合、警告メッセージを表示
                    short_card = (original_card[:40] + '...') if len(original_card) > 40 else original_card
                    warning_msg = f"警告: カード「{short_card}」から空の結果が返されました。プロンプトがモデルに合っていない可能性があります。\n"
    ...
    def conversion_thread(self):
        try:
            ...
            # LLMハンドラを初期化 (キャンセルイベントを渡す)
            handler = LLMHandler(config, self.cancel_event)
            ...
            for i in range(0, total_cards, batch_size):
                ...
                # LLMからストリーミングされる結果をリアルタイムで処理
                for idx, result in enumerate(handler.convert_cards(batch_cards)):
                    # 現在処理中のオリジナルカードを取得
                    original_card = batch_cards[idx]
                    self.queue.put(("result_part", result, original_card))

                    processed_cards += 1
                    self.queue.put(("progress", processed_cards))

                    # ハンドラ内でキャンセルされた場合、こちらもループを抜ける
                    if self.cancel_event.is_set():
                        break
