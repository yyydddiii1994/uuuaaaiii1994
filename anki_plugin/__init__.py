...
class MainDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        info_label = QLabel(
            "レビュー回数5回以上のカードを対象に、「分析実行」ボタンで認知科学に基づき負荷が高いカードを分析します。\n"
            ...
            "<li><b>想起の流暢性:</b> log(平均レビュー時間[秒] + 1) * 20</li></ul>"
        )
    ...
    def calculate_new_score(self, lapses: int, reps: int, factor: int, ivl: int, avg_time_sec: float) -> float:
        ...
        # レビュー時間の影響を対数スケールで緩やかにする
        retrieval_fluency = math.log(avg_time_sec + 1) * 20
    ...
    def analyze_cards(self):
        ...
        try:
            ...
            # レビュー回数が5回以上のカードのみを対象とする
            cards_data = mw.col.db.all("SELECT id, nid, reps, lapses, factor, ivl FROM cards WHERE reps >= 5")
...
def show_main_dialog():
    """メインダイアログをノンモーダルで表示する"""
    # 既にインスタンスが存在する場合は、それをアクティブにして前面に表示
    if hasattr(mw, 'cognitive_load_analyzer_dialog') and mw.cognitive_load_analyzer_dialog.isVisible():
        mw.cognitive_load_analyzer_dialog.activateWindow()
        return

    # インスタンスが存在しないか、非表示の場合は新規作成
    mw.cognitive_load_analyzer_dialog = MainDialog(mw)
    mw.cognitive_load_analyzer_dialog.show()
