...
このツールは、SteamのWeb API、またはPC内のローカルファイルを使用して、あなたのプレイ時間、所有ゲーム数、そして全ゲームの解除実績総数を集計・表示します。
...
### APIモード（推奨）
全ての機能（実績集計含む）が利用可能です。
1. **Steam API Key** と **Steam ID (64bit)** を入力します。
2. 「データ取得開始」ボタンを押します。

### ローカルモード
APIキーなしで、インストール済みのSteamからデータを読み込みます。
※ **実績データは取得できません**（プレイ時間とゲーム一覧のみ）。
1. 「ローカル読込」ボタンを押します。
...
## 注意事項
- **APIモード**: プロフィール設定で「ゲームの詳細」が「公開」になっていないと、データを取得できません。Steamのプライバシー設定を確認してください。
- **ローカルモード**: 実績数は集計されません。また、アンインストール済みのゲームなど、一部のゲーム名が正しく取得できない場合があります（AppIDで表示されます）。
